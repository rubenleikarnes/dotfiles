...
download()
{
	...
	if [ -f /etc/alpine-release ]; then
		...
		if [ -s "$VSCODE_REMOTE_BIN/compatibilty-check" ]; then
...
if [ ! -d "$VSCODE_REMOTE_BIN/$COMMIT" ]; then
	...
	CHECK_MARKER_FILE="$VSCODE_REMOTE_BIN/compatibilty-check"
	if [ ! -f "$CHECK_MARKER_FILE" ]; then
		...
		if [ -f "$CHECK_SCRIPT" ]; then
			...
			case $RC in
				99)
					...
					exit 99;;
				100)
					echo "Requires legacy code server";
					echo "legacy" > "$CHECK_MARKER_FILE";
					echo "Removing previous installation...";
					mv "$VSCODE_REMOTE_BIN/$COMMIT" "$VSCODE_REMOTE_BIN/$COMMIT-incompatib"
					exit 100;;
				*)
					echo "Compatibility check successful ($RC)";
					echo "" > "$CHECK_MARKER_FILE";
			esac
		fi
	fi
fi